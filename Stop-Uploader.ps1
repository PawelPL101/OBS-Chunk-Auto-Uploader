...
while ($true) {
    ...
    # The log is the source of truth for whether an upload is genuinely active.
    # progress.txt can hold a stale "UPLOADING|..." line if a previous upload was
    # interrupted (e.g. a cloud outage or daily quota block), so we only treat the
    # upload as active when the log's latest activity ALSO indicates uploading.
    # This guarantees the dashboard self-corrects to "(nothing uploading)" when
    # idle, with no manual restart or file cleanup needed.
    $logSaysUploading = ($lastLine -match "UPLOAD START")

    if ($prog.Active -and $logSaysUploading) {
